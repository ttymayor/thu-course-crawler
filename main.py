...
def main() -> None:
    ...
    course_detail_df = fetch_course_detail(academic_year, academic_semester, course_codes)
...
def fetch_course_info(academic_year: str, academic_semester: str) -> pd.DataFrame:
    try:
        response = requests.get(f"https://course.thu.edu.tw/opendatadownload/list/{academic_year}/{academic_semester}/")
...
def fetch_course_detail(academic_year: str, academic_semester: str, course_codes: List[str]) -> pd.DataFrame:
    ...
    for course_code in course_codes:
        try:
            ...
            # 獲取授課教師
            teacher_list = []
            teacher_section = soup.select_one('#mainContent > div:nth-child(4) > div:nth-child(1)')
            if teacher_section:
            ...
            if basic_info_element:
                ...
                lines = [line.strip() for line in basic_info_text.split('\n') if line.strip()]
                ...
                for line in lines:
                    if '：' in line:
                        ...
                        if key == '選修課' or key == '必修課':
                            ...
                            # 處理 "必修課，學分數：3-0" 這種格式
                            if '，學分數' in line:
                                credits_part = line.split('，學分數：')[1] if '，學分數：' in line else value
